...
RLAPI void DrawTextureQuad(Texture2D texture, Vector2 tiling, Vector2 offset, rlRectangle quad, Color tint);  // Draw texture quad with tiling and offset parameters
RLAPI void DrawTextureTiled(Texture2D texture, rlRectangle source, rlRectangle dest, Vector2 origin, float rotation, float scale, Color tint); // Draw part of a texture (defined by a rectangle) with rotation and scale tiled into dest.
...
RLAPI void DrawCubeTextureRec(Texture2D texture, rlRectangle source, Vector3 position, float width, float height, float length, Color color); // Draw cube with a region of a texture
